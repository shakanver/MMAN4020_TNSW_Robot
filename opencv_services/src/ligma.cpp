...
void apply_cv_algorithms(cv::Mat camera_image) {
  ...
  // set gloabl variable box_centroid
  box_centroid = cv::Point2f(maxLoc.x + cone_img.cols/2.0, maxLoc.y + cone_img.rows/2.0);
  ...
  cv::waitKey(3);
  // set gloabl variable target_centroid
  target_centroid = cv::Point2f(maxLoc.x + spigot_hole_img.cols/2.0, maxLoc.y + spigot_hole_img.rows/2.0);

  
  ROS_INFO_STREAM("2D box position pixel coords: x " << box_centroid.x << " y " << box_centroid.y);
  ROS_INFO_STREAM("2D target position pixel coords: x " << target_centroid.x << " y " << target_centroid.y);
